...
def read_dict_info():
    """
    """
    ifo_file = "./dict/ifo.ifo"
    idx_file = "./dict/idx.idx"
    dict_file = "./dict/dict.dict.dz"
    ...
    res = dict_reader.get_dict_by_word("have")
    res = res[0]['x'].replace('<blockquote>', '     ').replace('</blockquote>', '')
    print(res)

# read_ifo_file("stardict-cedict-gb-2.4.2/cedict-gb.ifo")
# read_idx_file("./test.idx")
read_dict_info()