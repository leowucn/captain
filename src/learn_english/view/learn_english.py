...
def show_word_date_list():
    if request.method == 'POST':
        ...
        word_date_list_from_word_builder = []
        word_date_list_from_clipboard = []
        year = request.form.keys()[0].split('-')[0]

        if year in res[0]:
            for month, words_dict in res[0][year].iteritems():
                word_date_list_from_word_builder.append(month)
        if len(res) > 1:
            if year in res[1]:
                for month, words_dict in res[1][year].iteritems():
                    word_date_list_from_clipboard.append(month)
        return render_template('word_date_list.html', year=year, word_date_list_from_word_builder=sorted(word_date_list_from_word_builder), word_date_list_from_clipboard=sorted(word_date_list_from_clipboard))
